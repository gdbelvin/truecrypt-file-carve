...
namespace TrueCrypt {
Volume::Volume() :
		HiddenVolumeProtectionTriggered(false), SystemEncryption(false), VolumeDataSize(
				0), TopWriteOffset(0), TotalDataRead(0), TotalDataWritten(0) {
}
...
void Volume::CheckProtectedRange(uint64 writeHostOffset, uint64 writeLength) {
	...
	if ((writeHostOffset < ProtectedRangeStart) ?
			(writeHostEndOffset >= ProtectedRangeStart) :
...
shared_ptr <File> Volume::OpenFile (
...
void Volume::Open(const VolumePath &volumePath, bool preserveTimestamps,
		shared_ptr<VolumePassword> password, shared_ptr <KeyfileList> keyfiles, VolumeProtection::Enum protection, shared_ptr <VolumePassword> protectionPassword, shared_ptr <KeyfileList> protectionKeyfiles, bool sharedAccessAllowed, VolumeType::Enum volumeType, bool useBackupHeaders, bool partitionInSystemEncryptionScope)
		{
			make_shared_auto (File, file);
...
void Volume::ReadSectors(const BufferPtr &buffer, uint64 byteOffset) {
	...
	EA->DecryptSectors(buffer, hostOffset / SectorSize, length / SectorSize,
			SectorSize);

	TotalDataRead += length;
}

void Volume::ReEncryptHeader(bool backupHeader, const ConstBufferPtr &newSalt,
		const ConstBufferPtr &newHeaderKey, shared_ptr<Pkcs5Kdf> newPkcs5Kdf)
		{
			if_debug (ValidateState ());
...
void Volume::WriteSectors(const ConstBufferPtr &buffer, uint64 byteOffset) {
	...
	EA->EncryptSectors(encBuf, hostOffset / SectorSize, length / SectorSize,
			SectorSize);
