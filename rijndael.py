...
class Rijndael:
    ...
    def decrypt(self, data):
        ...
        plain = b''
        for d in range(dlen // BLOCK_SIZE):
...
def setkey(rinst, key):
    ...
    j = 0
    for i in range(rinst.Nk):
        CipherKey[i] = pack(key[j:j+4]) # XXX
        j += 4
    for i in range(rinst.Nk):
        rinst.fkey[i] = CipherKey[i]
    k = 0
    for j in range(rinst.Nk, N, rinst.Nk):
        ...
        if rinst.Nk <= 6:
            ...
            while i < rinst.Nk and (i + j) < N:
                ...
                i += 1
        k += 1
    for j in range(rinst.Nb):
        rinst.rkey[j + N - rinst.Nb] = rinst.fkey[j]
    for i in range(rinst.Nb, N - rinst.Nb, rinst.Nb):
        k = N - rinst.Nb - i
        for j in range(rinst.Nb):
            rinst.rkey[k + j] = InvMixCol(rinst.fkey[i + j])
    for j in range(N - rinst.Nb, N):
        rinst.rkey[j - N + rinst.Nb] = rinst.fkey[j]
    return

def encrypt(rinst, buff):
    assert bytes == type(buff)
    buff = [b for b in buff]
    a = [0]*8
    b = [0]*8
    j = 0
    for i in range(rinst.Nb):
        a[i] = pack(buff[j:j+4])
        a[i] ^= rinst.fkey[i]
        j += 4
    k = rinst.Nb
...
if BLOCK_SIZE == 16:
    assert Rijndael(b'012345678abcdefgh00112233xyzqwer').encrypt(b'a'*16) == b'%\x98\x8a \xf8\\\x10\x9c\x17\x16\x9bb\x9e\xd6*\x96'
    assert Rijndael(b'012345678abcdefgh00112233xyzqwer').decrypt(b'%\x98\x8a \xf8\\\x10\x9c\x17\x16\x9bb\x9e\xd6*\x96') == b'a'*16
    assert Rijndael(b'\x10'*32).encrypt(b'1234'*4) == b'\xba\xad\xaawV|S\xc36>1\x03\xfd\x9e+\x9d'
